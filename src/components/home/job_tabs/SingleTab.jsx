...
const SingleTab = ({ job }) => {
  ...
  return (
    <motion.div
      whileHover={{ scale: 1.05 }} // Scales up the card slightly when hovered
      className="border py-3 px-4 rounded space-y-1"
      transition={{ type: "spring", stiffness: 300 }} // Optional: adjust the type of animation and its stiffness
    >
      <h1 className="text-2xl text-blue-400 font-medium">{job_title}</h1>
      <p>{name}</p>
      <p>Annual salary: ${firstSalary}-${secondSalary}</p>
      ...
      <div className="flex justify-between">
        ...
        <Link to={`/singlejobdetails/${_id}`}>
          <button className="px-3 rounded text-blue-400 hover:bg-blue-400 hover:text-white border">Details</button>
        </Link>
      </div>
    </motion.div>
  );
